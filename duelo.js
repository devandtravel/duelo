import board from './board.json' assert { type: 'json' }
...
DUELO.arc = function (start, end) {
  ...
  that.overlaps_range = function (range) {
    ...
    for (i = 0; i < ranges.length; i += 1) {
      if (
        (start >= ranges[i][0] && start <= ranges[i][1]) ||
...
DUELO.board = (function () {
  ...
  that.hexagonsInDistance = function (h, d) {
    ...
    return list.filter(function l(h) {
      return (
        h[0] >= 0 && h[0] <= dimensions[0] && h[1] >= 0 && h[1] <= dimensions[1]
      )
    })
  }
...
DUELO.player = function () {
  ...
  that.turnToHexagon = function (h, callback) {
    ...
    if (pos[0] !== i || pos[1] !== j) {
      new_direction = that.directionFromAngle(
        DUELO.board.angle([pos[0], pos[1]], [i, j])
      )
...
function game() {
  ...
  function createObstacles() {
    DUELO.board.obstacles = board.obstacles
    // var n = 40

    // for (; n > 0; n -= 1) {
    //   DUELO.board.obstacles.push([
  ...
  function step() {
    ...
    for (i = 0; i < DUELO.board.obstacles.length; i += 1) {
      $(
        '#hex_' +
          DUELO.board.obstacles[i][0] +
          '_' +
          DUELO.board.obstacles[i][1]
      ).attr({ fill: '#000' })
    }
  }
